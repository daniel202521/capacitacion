...
async function ensureConnected() {
	...
	try {
		...
		// NEW: items and tickets collections
...
// VAPID_PUBLIC_KEY = 'BE3OGd8E0TxFDNvAL85myO8GEFwkOhqOrkfqiJbXXveQQkpNF3_HwmWrd5SemRV9SN9EXXe1ZPFET0hnDcw2-Uc';
// VAPID_PRIVATE_KEY = '8PxGNwSHAy-_Fb55XlpY5NGN3N2VeNXfxXJuTcw93s'; // <-- Reemplaza por una clave privada VAPID válida
...
app.post('/api/take', async (req, res) => {
	try {
		...
		if(!item) return res.status(400).json({ error:'Artículo no encontrado' });
		if(Number(qty) > Number(item.quantity)) return res.status(400).json({ error:'No hay suficiente cantidad disponible' });

		// decrement atomically
		await itemsCol.updateOne({ id: itemId }, { $inc: { quantity: -Number(qty) } });

		const ticket = {
			id: 't' + Date.now(),
			type: 'take',
			itemId, username, qty: Number(qty), destination: destination||'', signature: signature||null,
			date: new Date().toISOString()
		};
		...
		console.error('POST /api/take error:', err);
		res.status(500).json({ error:'Error del servidor' });
	}
});
...
app.post('/api/return', async (req, res) => {
	try {
		...
		// compute userTaken from ticketsCol
		const userTickets = await ticketsCol.find({ username, itemId }).toArray();
		const userTaken = (userTickets || []).reduce((acc,t) => acc + (t.type === 'take' ? Number(t.qty) : -Number(t.qty)), 0);

		if(!force && Number(qty) > Number(userTaken)) return res.status(400).json({ error:'La persona no tiene esa cantidad para devolver' });
		...
		const ticket = {
			id: 't' + Date.now(),
			type: 'return',
			itemId, username, qty: Number(qty),
			destination: (destination == null ? '' : destination),
			signature: signature||null,
			date: new Date().toISOString(),
			forcedReturn: !!force,
		...
		console.error('POST /api/return error:', err);
		res.status(500).json({ error:'Error del servidor' });
	}
});

// helper: emitir actualizaciones usando la instancia `io` y leyendo desde DB
async function emitAll(){
	try {
		if(!itemsCol || !ticketsCol) return;
